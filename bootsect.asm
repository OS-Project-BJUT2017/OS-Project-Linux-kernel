...
# First things first. Move ourself from 0x7C00 -> 0x90000 and jump there.
	...
	movw	$BOOTSEG, %ax
	movw	%ax, %ds		# %ds = BOOTSEG ;将ds设为0x7C0
	movw	$INITSEG, %ax
	movw	%ax, %es		# %ax = %es = INITSEG ;将es段寄存器置为0x9000
	movw	$256, %cx
	subw	%si, %si
	subw	%di, %di
	cld
...
# and %gs is unused.
	...
	pushw	%ds
	ldsw	%fs:(%bx), %si		# %ds:%si is source ;LDS或LES指令是将双字的内存单元的值同时装载到段地址
